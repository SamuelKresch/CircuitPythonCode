...
from arducam_helper import *

class ArducamClass(object):
    #list 1 is [SCK,MOSI,MISO,CS], list 2 is [SCL,SDA]
    def __init__(self,SPI_PINS,I2C_PINS, CAMERA_MODE = JPEG,CAMERA_TYPE = Type):
        self.CameraMode = CAMERA_MODE
        self.CameraType = CAMERA_TYPE
        self.SPI_CS=digitalio.DigitalInOut(SPI_PINS[3])
        self.SPI_CS.direction = digitalio.Direction.OUTPUT
        self.I2cAddress=0x30
        self.spi = busio.SPI(clock=SPI_PINS[0], MOSI=SPI_PINS[1], MISO=SPI_PINS[2])
        while not self.spi.try_lock():
            pass
        self.spi.configure(baudrate=4000000,polarity=0,phase=0,bits=8)
        self.i2c = busio.I2C(scl=I2C_PINS[0], sda=I2C_PINS[1],frequency=1000000)
